...
class gameObject():
    def __init__(self, engine, pos=None,animation=None,sprite=None,sfx=None,shape=None,collider=None,visible = True,nocollide = False, active=True):
        self.engine = engine
        self.animations = [animation] if animation is not None else []
        self.sprites = [sprite]  if sprite is not None else []
        self.sfxs = [sfx] if sfx is not None else []
        ...
        self.active = active
        self.shapes = [shape] if shape is not None else []
        self.collider = [collider] if collider is not None else []
        self.collisions = []
        self.velocity = vec2(0,0)
        self.visible = visible
    def playSound(self,id,channel,gain):
        self.sfxs[id].play(channel,gain)
    def update(self,screen): # Edit this when inheriting
        if self.visible:
